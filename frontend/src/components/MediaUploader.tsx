...
import { Upload, X, Loader2, FileAudio, FileVideo, FileImage } from 'lucide-react';
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (selectedFile.size > MAX_FILE_SIZE) {
        onError("File is too large. Maximum allowed size is 50MB.");
        return;
      }
  ...
  if (isAnalyzing) {
    return (
      <div className="upload-container analyzing">
        ...
        <h3 className="pulse-text">Analyzing...</h3>
        <p className="subtitle">Please wait while the AI processes the {mode}.</p>
      </div>
    );
  }
